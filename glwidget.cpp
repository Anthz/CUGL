...
void GLWidget::initializeGL()
{
	...
	Shader *shader = new Shader("Main Shader");
...
void GLWidget::paintGL()
{
	...
	//QMatrix4x4::inverted(viewMatrix) //returns inverse matrix (camera position)

	if(GLSettings::ObjectList.size() > 0)
	{
		void *devPtr = CUGLBuffer::MapResource(GLSettings::BufferList.at(2)->CudaBuf());

		CUExecuteKernel(devPtr, GLSettings::ObjectList.at(0)->instances);	//hardcoded for 1 object
		CUGLBuffer::UnmapResource(GLSettings::BufferList.at(2)->CudaBuf());
...
void GLWidget::resizeGL(int w, int h)
{
	...
	projMatrix->perspective(45.0f, width / float(height), 0.01f, 1000.0f);
}
